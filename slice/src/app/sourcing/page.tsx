...
import { Plus, Search, ChevronDown, FileText, Upload } from "lucide-react";
...
export default function SourcingPage() {
  ...
  const [sourcedDeals, setSourcedDeals] = useState([
    {
      id: 1,
      dealName: "TechCorp Acquisition",
      company: "TechCorp Industries",
      sourceMethod: "Database Search",
      sourcedBy: "Pierre Marie",
      date: "Dec 18, 2024",
      status: "Qualified" as "New" | "Qualified" | "Contacted" | "Lost"
    },
    {
      id: 2,
      dealName: "Manufacturing Plus Exit", 
      company: "Manufacturing Plus",
      sourceMethod: "Import",
      sourcedBy: "Sarah Chen",
      date: "Dec 15, 2024",
      status: "Contacted" as "New" | "Qualified" | "Contacted" | "Lost"
    },
    {
      id: 3,
      dealName: "Green Energy Merger",
      company: "Green Energy Solutions",
      sourceMethod: "Database Search",
      sourcedBy: "Alex Johnson",
      date: "Dec 20, 2024",
      status: "New" as "New" | "Qualified" | "Contacted" | "Lost"
    },
    {
      id: 4,
      dealName: "Retail Chain Sale",
      company: "Retail Dynamics",
      sourceMethod: "Import",
      sourcedBy: "Pierre Marie",
      date: "Dec 12, 2024",
      status: "Lost" as "New" | "Qualified" | "Contacted" | "Lost"
    }
  ]);
  const modalRef = useRef<HTMLDivElement>(null);

  const handleStatusClick = (deal: typeof sourcedDeals[0], event: React.MouseEvent) => {
  ...
  const handleStatusChange = (dealId: number, newStatus: "New" | "Qualified" | "Contacted" | "Lost") => {
    setSourcedDeals(prev => 
      prev.map(d => 
        d.id === dealId 
          ? { ...d, status: newStatus }
          : d
  ...
  return (
    <div className="flex flex-1 flex-col h-full">
      ...
      <div className="bg-gray-50 px-4 pt-15 pb-4 flex-shrink-0">
        ...
        {/* Grille des 2 cartes principales */}
        <div className="grid grid-cols-2 gap-6 mb-4">
          {/* Card Find Targets */}
          <Link href="/companies">
            <div className="bg-white border border-gray-200 rounded p-6 h-20 hover:border-gray-300 transition-colors cursor-pointer">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-blue-50 rounded">
                  <Search className="h-4 w-4 text-blue-600" />
                </div>
                <h3 className="font-medium text-gray-900 text-sm">Find Targets</h3>
              </div>
            </div>
          </Link>
          ...
          <div className="bg-white border border-gray-200 rounded p-6 h-20 hover:border-gray-300 transition-colors cursor-pointer">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-medium text-gray-900 text-sm">Import Targets</h3>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="h-full">
          ...
          <div className="flex items-center justify-between pb-4">
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-medium text-gray-900">Recently Sourced Targets</h2>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search sourced deals..."
                  value={searchQuery}
          ...
          <div className="bg-white">
            ...
            <div className="grid grid-cols-12 gap-4 px-4 py-3 text-xs font-medium text-gray-500 border-b border-gray-200">
              ...
              <div className="col-span-2">Sourced By</div>
            ...
            <div className="divide-y divide-gray-100">
              {sourcedDeals.map((deal) => (
                <div
                  key={deal.id}
                  className="grid grid-cols-12 gap-4 px-4 py-2 hover:bg-gray-50 transition-colors group cursor-pointer"
                ...
                >
                  ...
                  <div className="col-span-2 flex items-center">
                    <span className="text-xs text-gray-600">{deal.sourceMethod}</span>
                  </div>
                  
                  <div className="col-span-2 flex items-center">
                    <span className="text-xs text-gray-600">{deal.sourcedBy}</span>
                  </div>
            ...
            {/* CTA pour nouveau deal sourcé - style Clay minimaliste */}
            <div className="mt-2">
              <Link href="/sourcing" className="flex items-center gap-1.5 px-2 py-1 text-gray-500 hover:text-gray-700 hover:bg-gray-50 rounded transition-colors group">
                <Plus className="h-3.5 w-3.5" />
                <span className="text-xs font-medium">New sourced deal</span>
              </Link>
            </div>
          </div>
        </div>
          ...
          >
            <div className="py-1">
              <div
                ...
                onClick={() => handleStatusChange(openStatusModal, "New")}
              >
                <div className="text-xs font-medium text-gray-900">New</div>
              </div>
              
              <div
                className="px-3 py-2 transition-colors cursor-pointer hover:bg-gray-50"
                onClick={() => handleStatusChange(openStatusModal, "Qualified")}
              >
                <div className="text-xs font-medium text-gray-900">Qualified</div>
              </div>
              ...
              <div
                className="px-3 py-2 transition-colors cursor-pointer hover:bg-gray-50"
                onClick={() => handleStatusChange(openStatusModal, "Lost")}
              >
                <div className="text-xs font-medium text-gray-900">Lost</div>
              </div>
            </div>
          </div>
        </>
