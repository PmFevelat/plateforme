...
import { X, Building2, Users, MapPin, DollarSign, Calendar } from 'lucide-react';
...
const MarketGraph: React.FC = () => {
  ...
  // Appliquer des styles personnalisés pour la scrollbar
  useEffect(() => {
    if (panelRef.current) {
      ...
      panelRef.current.classList.add('scrollbar-thin');
  ...
  return (
    <div className="relative w-full h-full bg-white overflow-hidden">
      ...
      {/* Cards flottantes à droite */}
      {showSidePanel && selectedCompany && (
        <div 
          className="fixed top-16 right-4 bottom-4 w-80 z-20 pointer-events-none"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Bouton fermer fixe et discret */}
          <button
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
            ...
            className="absolute top-2 right-2 z-30 bg-black/20 hover:bg-black/30 backdrop-blur-sm rounded-full p-1.5 text-white/70 hover:text-white transition-all duration-200 pointer-events-auto"
          >
          ...
          >
            <div className="space-y-4">
              ...
              {/* Card informations de la cible */}
              <div className="bg-white/80 backdrop-blur-md rounded-lg shadow-sm border border-gray-200/50 p-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span>{selectedCompany.revenue} revenue</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span>Last contact: {new Date(selectedCompany.lastContact).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>

              {/* Card acheteurs potentiels */}
              {potentialBuyers.length > 0 && (
                <div className="bg-white/80 backdrop-blur-md rounded-lg shadow-sm border border-gray-200/50 p-4">
                  <h4 className="font-medium text-gray-900 mb-3">Potential Buyers ({potentialBuyers.length})</h4>
                  <div className="space-y-3">
                    {potentialBuyers.map((buyer) => (
                      <div key={buyer.id} className="bg-white/60 backdrop-blur-sm border border-gray-200/30 rounded-lg p-3">
                        <h5 className="font-medium text-gray-900 text-sm mb-2">{buyer.name}</h5>
              ...
              {/* Padding bottom pour éviter que le dernier élément soit collé */}
